...
func CloneReflect(v interface{}) interface{} {
	if v == nil {
		return nil
	}
	...
	//return clonereflect(dst, src).(*FlexObj)

	clonereflect(dst, src)

	return dst.Interface()
}

func clonereflect(dst reflect.Value, src reflect.Value) {
	switch src.Kind() {
	case reflect.Ptr:
		srcVal := src.Elem()
	...
	case reflect.Struct:
		...
		dstMethod := dst.Addr().MethodByName("GetNumOfField")

		if dstMethod.IsValid() != true {
			return
		}

		//fff := dst.Addr().GetNumOfField()
		//fmt.Printf("fff: %v\n", fff)
		...
		for i := 0; i < src.NumField(); i++ {
			...
			//fmt.Printf("FN: %v\n", src.Field(i).Kind())
			//clonereflect(dst.Field(i), src.Field(i))
		}
